...
- [~] **Caching** (L): **done** — a **RAM frame cache** (frame-indexed, `PreviewRenderer`) holds each rendered preview frame so the **work-area loops back in real time** once filled (the *RAM-preview* model); invalidated by a `(comp-state hash, render dims)` signature with a cache **epoch** that skips superseded in-flight renders, and bounded by a ~1 GiB **byte budget** with farthest-from-playhead eviction (smart-ish purge). A subtle **"caching… N%"** transport readout (`PreviewRenderer::cache_progress`) shows the first-pass fill (AE's green cache bar), so the expected "laggy first play, smooth on loop" behaviour reads as intentional. Still TODO: **disk cache** + persistence across sessions, finer purge heuristics, cache-on-render (share the export render into the preview cache), and a per-frame cache bar on the timeline ruler
