...
| Precomps / parenting | nesting, collapse, pick-whip | **Partial** (precomp **nesting** [recursive render + cycle guard + time-offset], **pre-compose** [single layer], pick-whip **parenting** done; **collapse transformations**, multi-layer pre-compose, comp navigator, time-remap curve **Planned**) | 2 |
...
4. [~] **Layer types**: Adjustment + Null + **footage** (stills + image sequences via `prism-io`) + **precomps** (nest a sibling comp, recursive render + cycle guard + time-offset, single-layer pre-compose) done; **footage video** (FFmpeg / `prism-media`), **precomp collapse-transformations / time-remap curve / multi-layer pre-compose** (Ph2/Ph4) still TODO.
